...
function getElementWidth(element) {
  let toReturn = element.getBoundingClientRect();
  return toReturn.width;
}
...
function renderPaletteRow(colorStrings, parentDom) {
  let newDiv = document.createElement("div");
  let parentSiz = getElementWidth(parentDom);
  let sizeArr = colorStrings.length;
  let number = parentSiz / sizeArr;
  let boxSiz = parentSiz / number;
  for(let i = 0; i < sizeArr; i++) {
    let toAppend = createColorBox(colorStrings[i], boxSiz);
    newDiv.append(toAppend);
  }
  parentDom.append(newDiv);
}
...
function renderPaletteTable() {
  ...
  for (const color in COLORS_9) {
    renderPaletteRow(COLORS_9[color], parentDom);
  }
}
...
let toRemove = document.querySelector('header p');
